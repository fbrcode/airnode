...
async function deploy(
  ...
  if (!(await stateExists(region, bucket, dynamodbTable))) {
    ...
    command = `terraform apply -var="aws_region=${region}" -var="airnode_id_short=${airnodeAddressShort}" -var="stage=${stage}" -auto-approve -input=false -no-color`;
    await runCommand(command, options);
  }
...
async function remove(airnodeAddressShort: string, stage: string, region: string) {
  ...
  command = `terraform destroy -var="aws_region=${region}" -var="airnode_id_short=${airnodeAddressShort}" -var="stage=${stage}" -var="configuration_file=X" -var="secrets_file=X" -var="handler_file=${handlerFile}" -auto-approve -input=false -no-color`;
  await runCommand(command, options);
